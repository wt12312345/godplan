package com.hq.manage.controller;
...
import com.hq.base.util.MD5Util;
import com.hq.base.util.TypeUtil;
import com.hq.base.util.WebUtil;
import com.hq.entity.UserSys;
import com.hq.manage.SessionUtil;
import com.hq.manage.service.UserSysService;
import com.hq.manage.vo.UserSysVo;
import com.hq.web.controller.AbstractController;
...
public class UserSysController extends AbstractController {
	...
	public JsonResponse doLogin(HttpServletRequest request, HttpServletResponse response, HttpSession session) {
		...
		try {
			...
			if (userDb == null) {
				...
				session.setAttribute(SessionName.SysUser, userSysVo);
	...
	public JsonResponse checkLogin(HttpServletRequest request, HttpServletResponse response, HttpSession session) {
		JsonResponse jr = new JsonResponse();
		try {
			UserSysVo userSys = SessionUtil.getSystemManager(request);
			if (userSys != null) {
				jr.setCode(BegCode.SUCCESS);
				jr.setObj(userSys);
				jr.setMsg("已登录");
	...
	public JsonResponse changePassword(HttpServletRequest request, HttpServletResponse response) {
		...
		if (cb.getResult()) {
			logger.info("用户：" + SessionUtil.getSystemManager(request).getName() + " 修改密码");
			jr.setCode(BegCode.SUCCESS);
		} else {
			jr.setMsg(cb.getErr());
		}
