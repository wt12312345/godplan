...
<body>
					...
					<form name="frmData" id="frmData" method="post">
						<table class="table table-hover">
							...
							<tbody>
								<tr>
									<th>ID</th>
									<td><input type="text" class="form-control" name="id" id="id" value="${item.id }"
										readonly="readonly" />
								</tr>
								<tr>
									<th>菜单名称</th>
								...
								<tr>
									<th>符号class</th>
									<td><input type="text" class="form-control" name="iconName" id="iconName" placeholder="1级菜单必填"
										value="${item.iconName }" />
								</tr>
								<tr>
									<th>排列序号</th>
									<td><input type="text" class="form-control" name="sortIndex" id="sortIndex"
	...
	<script type="text/javascript">
		function doSave() {
			...
			$.post("${path}m/sys/saveMenu", args, function(data) {
				if (data.code == 1) {
					wtCom.OpenUrl("${path}m/sys/menuList");
				} else {
					alert(data.msg);
				}
			});
